...
`agent create <role> --harness codex|pi|claude --cwd <dir> [--instructions-file <path>]` 会创建带有会话专属 `AGENTS.md` 的持久化 profile；`agent profile <role>` 查看 profile 和文件路径，`agent profiles` 列出所有 profile。`agent spawn` 会创建或复用持久化角色 profile，并在 `--tab` 指定的标签页中启动；省略 `--tab` 时使用活动标签页。第一个实例使用角色名，后续仍存活的实例使用 `role-replica-N`。Herdr 会立即复用可用 shell；目标标签页没有可用 shell 时自动分割最后一个窗格。自动分割后，请求会最多轮询两秒等待新 shell 返回提示符，然后才提交智能体命令；如果未准备好，Herdr 会回滚新窗格并返回 `agent_spawn_failed`。`--kind` 是每次启动的显式覆盖；省略时使用已保存的 profile harness（默认 `codex`）。`--cwd tab` 使用目标标签页目录；`--cwd agent` 使用首次 spawn 时初始化的 profile native 目录。命令提交后，spawn 可能报告 `launch_pending`；它的 `--timeout` 是 3001–300000 毫秒的受管检测期限，而不是等待 ready。需要 ready 时使用 `agent wait <name> --until idle`。
