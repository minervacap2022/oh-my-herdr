...
- 通过 CLI 辅助命令列出、检查、读取、提示、等待、重命名、聚焦、启动、spawn 和附加智能体
- 从钩子和插件上报自定义智能体状态
- 订阅事件并等待输出或状态变化
- 安装和卸载内置集成
- 停止服务器并重载配置
...
| 智能体 | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.spawn`、`agent.profile.get`、`agent.profile.list`、`agent.profile.set`、`agent.profile.set_md` |
...
`agent.spawn` 会创建或复用 `role` 指定的持久化 profile，在选定的标签页中启动并返回 `.result.spawned`（省略 `tab_id` 时使用活动标签页）。通过 `tab_id` 选定后台标签页不会改变焦点。它会复用可用 shell；没有可用 shell 时自动分割最后一个窗格，并把后续仍存活的实例命名为 `role-replica-N`。`cwd_mode` 默认为 `tab`，设为 `agent` 时使用 profile 的 native 目录。`agent.profile.create` 可创建 codex、pi 或 claude profile，并同时创建当前会话专属的 `AGENTS.md`；Codex 使用 `model_instructions_file`，Pi/Claude 使用原生 prompt-file 参数。`agent.profile.get`、`agent.profile.list`、`agent.profile.set` 和 `agent.profile.set_md` 用于读取和更新 profile。spawn 会为 Claude、Codex、Pi 应用 model 和 effort，并为 Claude、Pi 应用 allowlist。`apikey_ref` 对 Claude 和 Codex 使用 `env:NAME` 环境变量引用。不支持的设置会明确失败，而不是静默忽略。
...
订阅客户端必须持续读取套接字。Herdr 会限制每条连接的可靠控制队列；该队列满时，Herdr 会退役客户端连接。连接被退役后，请重新连接并再次订阅。
