...
herdr agent broadcast <role> <text>
herdr agent roster
herdr agent create <role> --harness codex|pi|claude --cwd <dir> [--instructions-file <path>]
herdr agent profile <role>
herdr agent profiles
herdr agent revive <instance-id> [--tab TAB] [--cwd tab|agent] [--timeout MS]
herdr agent rename <target> <name>|--clear
herdr agent focus <target>
herdr agent wait <target> [--until STATUS]... [--timeout MS]
herdr agent attach <target> [--takeover]
herdr agent start <name> --kind KIND --pane ID [--timeout MS] [-- <agent-args...>]
herdr agent spawn <role> [--kind KIND] [--tab TAB] [--cwd tab|agent] [--timeout MS] [-- <agent-args...>]
herdr agent explain <target> [--json|--verbose]
herdr agent explain --file PATH --agent LABEL [--json|--verbose]
```

Agent targets are either a unique live agent name, a pane ID, or a persistent role target. Use `role@w:t:p` to select one replica and `agent broadcast <role> <text>` to address every live instance of a role. Terminal IDs and bare agent-kind labels are not agent targets. Agents started through `agent start` require a name; manually launched agents remain unnamed and use their pane ID.
...
`agent create <role> --harness codex|pi|claude --cwd <dir> [--instructions-file <path>]` creates a saved native profile and a session-owned `AGENTS.md` for its durable instructions. `agent profile <role>` shows that profile and the exact instruction-file path; `agent profiles` lists every saved profile. `agent spawn` creates or reuses a persistent role profile and launches it in the selected tab, or the active tab when `--tab` is omitted. The first instance uses the role name; later live instances receive `role-replica-N` names. Herdr uses an available shell immediately or auto-splits the last pane when the target tab has none. After auto-splitting, the request polls for the new shell prompt for up to two seconds before submitting the agent command; if it does not become ready, Herdr rolls back the new pane and returns `agent_spawn_failed`. `--kind` is an explicit per-launch override; without it, spawn uses the saved profile harness (default `codex`). `--cwd tab` uses the target tab directory; `--cwd agent` uses the profile's native directory, seeded on its first spawn. After command submission, spawn may report `launch_pending`; its `--timeout` is a managed detection deadline (3001–300000 milliseconds), not a readiness wait. Use `agent wait <name> --until idle` when readiness matters.
