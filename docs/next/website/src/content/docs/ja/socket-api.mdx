...
- CLI ヘルパーを通じたエージェントの一覧、調査、読み取り、プロンプト、待機、名前変更、フォーカス、起動、spawn、アタッチ
...
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send_keys`、`agent.prompt`、`agent.wait`、`agent.rename`、`agent.focus`、`agent.start`、`agent.spawn`、`agent.profile.get`、`agent.profile.list`、`agent.profile.set`、`agent.profile.set_md` |
...
`agent.spawn` は `role` で指定した永続プロファイルを作成または再利用し、選択したタブ（`tab_id` を省略した場合はアクティブなタブ）で起動して `.result.spawned` を返します。`tab_id` でバックグラウンドのタブを選択してもフォーカスは変わりません。利用可能なシェルを再利用し、なければ最後のペインを自動分割し、後続の live インスタンスには `role-replica-N` を付けます。`cwd_mode` は既定で `tab`、プロファイルの native ディレクトリを使う場合は `agent` です。`agent.profile.create` は codex、pi、claude のプロファイルとセッション専用 `AGENTS.md` を作成します。Codex は `model_instructions_file`、Pi/Claude はネイティブの prompt-file 引数で注入します。`agent.profile.get`、`agent.profile.list`、`agent.profile.set`、`agent.profile.set_md` でプロファイルを読み書きできます。spawn は Claude、Codex、Pi に model と effort を適用し、Claude と Pi に allowlist を適用します。`apikey_ref` は Claude と Codex で `env:NAME` の環境変数参照として使われます。対応しない設定は黙って無視せず失敗します。
...
購読クライアントはソケットを継続的に読み取る必要があります。Herdr は接続ごとの信頼性制御キューを上限付きにしており、そのキューが満杯になるとクライアント接続を退役させます。退役した接続の後は、再接続して再度購読してください。
