...
`agent create <role> --harness codex|pi|claude --cwd <dir> [--instructions-file <path>]` はセッション専用 `AGENTS.md` を持つ永続プロファイルを作成します。`agent profile <role>` はプロファイルと正確なファイルパスを表示し、`agent profiles` は全プロファイルを一覧します。`agent spawn` は永続ロールプロファイルを作成または再利用し、`--tab` で選んだタブ（省略時はアクティブなタブ）で起動します。最初のインスタンスはロール名を使い、後続の live インスタンスには `role-replica-N` が付きます。Herdr は利用可能なシェルをすぐに使い、対象タブにない場合は最後のペインを自動分割します。自動分割後は、新しい shell のプロンプトを最大 2 秒ポーリングしてからエージェントコマンドを送信します。準備できなければ、新しいペインをロールバックして `agent_spawn_failed` を返します。`--kind` は起動ごとの明示的な上書きで、省略時は保存済みプロファイルの harness（既定 `codex`）を使います。`--cwd tab` は対象タブのディレクトリを、`--cwd agent` は最初の spawn 時に設定されるプロファイルの native ディレクトリを使います。コマンド送信後、spawn は `launch_pending` を返すことがあります。`--timeout` は 3001–300000 ミリ秒の管理された検出期限であり、ready を待ちません。ready が必要なときは `agent wait <name> --until idle` を使います。
